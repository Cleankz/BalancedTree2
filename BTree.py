...
class BalancedBST:
    ...
    def createBalancedTree(self, arr, start, end,arr_res2 = []):
        if end < start:
            return None
        mid = (start + end) // 2
        node = arr[mid]
        arr_res2.append(node)
        node_left = self.createBalancedTree(arr, start, mid - 1,arr_res2)
        node_right = self.createBalancedTree(arr, mid + 1, end,arr_res2)
        return arr_res2

    def GenerateTree(self, a):
        a = sorted(a)
        a = self.createBalancedTree(a, 0, len(a) - 1)
